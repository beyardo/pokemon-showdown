...
export const BattleFormats: {[k: string]: FormatsData} = {
	...
	gigantamaxonlyclause: {
		effectType: 'Rule',
		name: 'Gigantamax Only Clause',
		desc: "Only allows Gigantamax mons to dynamax",
		onBegin() {
			for (const pokemon of this.getAllPokemon()) {
				if (pokemon.canGigantamax) {
					pokemon.canDynamax = true
				} else { pokemon.canDynamax = false };

				
			}
			this.add('rule', 'Only Gigantamax Pokemon may use the power of Dynamax in the IPL');
		},
	},
	dynamaxclause: {
		effectType: 'Rule',
